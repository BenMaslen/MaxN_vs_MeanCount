...
goal_3_dat_tb <- read_csv("results/goal_3_dat_tb.csv")
goal_3_dat_kf <- read_csv("results/goal_3_dat_kf.csv")
...
for(i in 1:length(split_data_kf)) {
  ...
  if(i == 1) {
    p <- p  + scale_y_log10(limits = c(goal_3_dat_kf_scale_eval$var_min[1], Avgn_ylim_var_kf)) +
      xlab("mean (log)") +
      ylab(expression(atop("MeanCount","variance (log)"))) + theme(plot.title = element_text(size = 12,hjust=0.5),
                                                                                       legend.position="none",axis.title.y = element_blank(),
...
ggsave("plots/goal3_plot_bias.png",height=1.1*5.12,width=1.1*5.79*(0.98/0.62))
...
ggsave("plots/goal3_plot_var.png",height=1.1*5.12,width=1.1*5.79*(0.98/0.62))
...
#for the appendix
ggplot(goal_3_dat_combined, aes(y=bias_rel,x=measure,fill=Class)) + geom_boxplot() + theme(legend.position="none") + 
  theme_bw() + ylab("relative bias") + guides(fill = "none") +facet_grid(vars(data))
