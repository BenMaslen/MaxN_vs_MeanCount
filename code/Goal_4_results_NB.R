...
goal_4_dat_kf$value_sum = goal_4_dat_kf$value*goal_4_dat_kf$frame_rate*56*60
...
goal_4_dat_tb$value_sum = goal_4_dat_tb$value*goal_4_dat_tb$frame_rate*60*goal_4_dat_tb$vid_length_new
...
kf_maxn_mod_full_temp <- glmmTMB(value~year*river,
                                 data=goal_4_dat_kf %>% filter(Class == species_kf[i],measure=="MaxN"),
                                 family=nbinom2,control=glmmTMBControl(optimizer=optim,optArgs = list(method="L-BFGS-B")))
kf_maxn_mod_null_temp <- glmmTMB(value~1,
                                 data=goal_4_dat_kf %>% filter(Class == species_kf[i],measure=="MaxN"),
                                 family=nbinom2,control=glmmTMBControl(optimizer=optim,optArgs = list(method="BFGS")))
...
sim= simulateResiduals(tb_maxn_mod_full_temp)
resid = residuals(sim, quantileFunction = qnorm, outlierValues = c(-4,4))
plot(resid~predict(tb_maxn_mod_full_temp, re.form = NA,type="response")) #concerning
...
sum(p_values_dat_tb$MaxN<0.05)
sum(p_values_dat_tb$AvgN<0.05)
sum(p_values_dat_kf$MaxN<0.05)
sum(p_values_dat_kf$AvgN<0.05)


write.csv(test_statistic_dat_kf,"results/goal_4_test_statistic_dat_kf.csv")  
write.csv(test_statistic_dat_tb,"results/goal_4_test_statistic_dat_tb.csv")  
write.csv(p_values_dat_kf,"results/p_values_dat_kf.csv")
write.csv(p_values_dat_tb,"results/p_values_dat_tb.csv")
