...
#parameter set - winner winner chicken dinner - no difference in mobility and schooling for meancount mean
# speed_ratio = 10
# overall_population_mean = 20
# p_in = 0.001
# p_out = 0.01
# 
# n_sims = 1000
...
for (j in 1:n_sims){
  ...
  for (i in 2:vid_length){
    ...
    MaxN_slow[(j-1)*(vid_length)+i] = max(N_vid[1:i])
    AvgN_slow[(j-1)*(vid_length)+i] = mean(N_vid[1:i])
  }
}
...
for (j in 1:n_sims){
  ...
  for (i in 2:vid_length){
    ...
    MaxN_fast[(j-1)*(vid_length)+i] = max(N_vid[1:i])
    AvgN_fast[(j-1)*(vid_length)+i] = mean(N_vid[1:i])
  }
}
...
for (j in 1:n_sims){
  ...
  for (i in 1:(vid_length-1)){
    ...
    MaxN_slow_school[(j-1)*(vid_length)+i+1] = max(N_vid[1:i])
    AvgN_slow_school[(j-1)*(vid_length)+i+1] = mean(N_vid[1:i])
  }
}
...
for (j in 1:n_sims){
  ...
  for (i in 1:(vid_length-1)){
    ...
    MaxN_fast_school[(j-1)*(vid_length)+i+1] = max(N_vid[1:i])
    AvgN_fast_school[(j-1)*(vid_length)+i+1] = mean(N_vid[1:i])
  }
}
...
write.csv(time_comp_long,"results/sim_goal_2_dat.csv")
