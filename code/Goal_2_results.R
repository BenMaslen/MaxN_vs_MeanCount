...
goal_2_dat_tb <- read_csv("results/goal_2_dat_tb_long.csv")
goal_2_dat_kf <- read_csv("results/goal_2_dat_kf_long.csv")
...
sim_goal_2_dat <- read.csv("results/sim_goal_2.csv")
...
Tassie_sp_vids <- goal_2_dat_combined_sims %>% filter(group=="Tassie - baited") %>%
  droplevels() %>% mutate(samp_levels=Video_name:Class) %>% select(samp_levels) %>% 
...
ggsave("plots/abund_video_length_sims.png",height=1.1*5.12,width=1.1*5.79*(0.98/0.62))
