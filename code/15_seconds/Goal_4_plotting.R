...
test_statistic_dat_kf = read.csv("results/60_seconds/goal_4_test_statistic_dat_kf.csv")  
test_statistic_dat_tb = read.csv("results/60_seconds/goal_4_test_statistic_dat_tb.csv")  
...
p_values_dat_kf <- read.csv("results/60_seconds/p_values_dat_kf.csv")
p_values_dat_tb <- read.csv("results/60_seconds/p_values_dat_tb.csv")
...
pow_4 <- read.csv("data/60_seconds/power_dat/power_10_20_4_1000.csv")
pow_8 <- read.csv("data/60_seconds/power_dat/power_10_20_8_1000.csv")
pow_12 <- read.csv("data/60_seconds/power_dat/power_10_20_12_1000.csv")
pow_16 <- read.csv("data/60_seconds/power_dat/power_10_20_16_1000.csv")
pow_20 <- read.csv("data/60_seconds/power_dat/power_10_20_20_1000.csv")
pow_24 <- read.csv("data/60_seconds/power_dat/power_10_20_24_1000.csv")
pow_28 <- read.csv("data/60_seconds/power_dat/power_10_20_28_1000.csv")
pow_32 <- read.csv("data/60_seconds/power_dat/power_10_20_32_1000.csv")
pow_36 <- read.csv("data/60_seconds/power_dat/power_10_20_36_1000.csv")
pow_40 <- read.csv("data/60_seconds/power_dat/power_10_20_40_1000.csv")
...
raw_12 <- read.csv("data/60_seconds/power_dat/raw_data_10_20_12_1000.csv") #need to create this!!!
...
test_stat_plots_alt[[2]] <- ggplot(test_statistic_dat_kf, aes(y = MaxN,x=AvgN,colour=Schooling:Mobility)) + 
  ...
  scale_y_continuous(limits=c(0,140),expand=c(0.025,0.025)) +
  scale_x_continuous(limits=c(0,140),expand=c(0.1,0.025)) +
  ylab(parse(text = expression({chi^2}~statistic ~~(MaxN)))) +  
  theme_classic()+  geom_abline(colour="black",linewidth=0.2,alpha=0.5) + 
  geom_hline(yintercept = 11.07,linetype="dashed",linewidth=0.3,alpha=0.5,color="#a590ad")+
  geom_vline(xintercept = 11.07,linetype="dashed",linewidth=0.3,alpha=0.5,color="#3e6478")+
  annotate(geom="text", x=11.07+4.8, y=122, label="p [MeanCount] < 0.05",
           color="#3e6478",size=3.5,parse=TRUE,angle=270)+
  annotate(geom="text", x=125, y=11.07+3.3, label="p [MaxN] < 0.05",
...
test_stat_plots_alt[[1]] <- ggplot(test_statistic_dat_tb, aes(y = MaxN,x=AvgN,colour=Schooling:Mobility)) + 
  ...
  geom_hline(yintercept = 11.07,linetype="dashed",linewidth=0.3,alpha=0.5,color="#a590ad")+
  geom_vline(xintercept = 11.07,linetype="dashed",linewidth=0.3,alpha=0.5,color="#3e6478")+
  annotate(geom="text", x=11.07+4, y=88, label="p [MeanCount] < 0.05",
           color="#3e6478",size=3.5,parse=TRUE,angle=270)+
  annotate(geom="text", x=85, y=11.07+2.5, label="p [MaxN] < 0.05",
           color="#a590ad",size=3.5,parse=TRUE)+
  theme(legend.position = "bottom",
        plot.title=element_text(size=12,hjust=0.5)) + 
...
strong_effect_species <- test_statistic_dat_kf$Class[abs(test_statistic_dat_kf$diff)>sort(abs(test_statistic_dat_kf$diff))[16]]
...
plot_cols_kf <- brewer.pal(n = 4, name = "Set2")[match(goal_4_dat_kf %>% filter(Class %in% strong_effect_species) %>% droplevels() %>% 
                                          ...
                                          unlist() %>% factor() %>% unique(),levels(goal_4_dat_kf$Schooling:goal_4_dat_kf$Mobility))]
...
plot_cols_tb2 <- brewer.pal(n = 4, name = "Dark2")[match(goal_4_dat_tb %>% filter(Class %in% strong_effect_species_tb) %>% droplevels() %>% 
                                                       mutate(Schooling_Mobility = Schooling:Mobility) %>% dplyr::select(Schooling_Mobility) %>% 
                                                       unlist() %>% factor() %>% levels(),levels(goal_4_dat_tb$Schooling:goal_4_dat_tb$Mobility))]



goal_4_plots_2[[1]] <- ggplot(goal_4_dat_tb %>% filter(Class %in% strong_effect_species_tb), aes(y=value_tol,x=bait,fill=bait:Schooling:Mobility)) + 
  geom_boxplot() + scale_y_log10() + facet_grid(measure~Class_short,scales = "free_y",switch = "y") +theme_bw() + 
  #scale_fill_manual(values = c("#B2DF8A", "#33A02C"),name = "Bait:") + ylab("value") +
  scale_fill_manual(values = c(plot_cols_tb, plot_cols_tb2),guide="none") + ylab("value") +
...
ggsave("plots/60_seconds/goal_4_combined_plot.png",height=1.1*5.12,width=1.1*5.79*(0.98/0.62))
...
ggsave("plots/60_seconds/goal_4_combined_plot2.png",height=1.1*5.12,width=1.1*5.79*(0.98/0.62))
