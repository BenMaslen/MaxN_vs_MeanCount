...
#read data
kf_4_min_dat_original <- read_csv("data/Kakadu_fish_pred.csv")
tb_4_min_dat_original <- read_csv("data/Tassie_BRUV_RUV_pred.csv")

kf_4_min_dat_skipped <- read_csv("data/60_seconds/Kakadu_fish_pred.csv")
tb_4_min_dat_skipped <- read_csv("data/60_seconds/Tassie_BRUV_RUV_pred.csv")

#check they are ordered the same
sum(kf_4_min_dat_original$Class!=kf_4_min_dat_skipped$Class & 
      kf_4_min_dat_original$Video_name!=kf_4_min_dat_skipped$Video_name &
      kf_4_min_dat_original$split_number!=kf_4_min_dat_skipped$split_number)

sum(tb_4_min_dat_original$Class!=tb_4_min_dat_skipped$Class & 
      tb_4_min_dat_original$Video_name!=tb_4_min_dat_skipped$Video_name &
      tb_4_min_dat_original$split_number!=tb_4_min_dat_skipped$split_number)

#replace original MeanCount with skipped MeanCount
kf_4_min_dat_original$`Average Objects per frame` <- kf_4_min_dat_skipped$`Average Objects per frame`
tb_4_min_dat_original$`Average Objects per frame` <- tb_4_min_dat_skipped$`Average Objects per frame`

#rename
kf_4_min_dat = kf_4_min_dat_original
tb_4_min_dat = tb_4_min_dat_original
...
kf_4_min_dat <- kf_4_min_dat %>% dplyr::select(-c("First appeared","First appeared prop"))
tb_4_min_dat <- tb_4_min_dat %>% dplyr::select(-c("First appeared","First appeared prop"))
...
kf_4_min_dat <- kf_4_min_dat_max_split_temp %>% filter(split_number>1,split_number<max_split-1) %>% 
  dplyr::select(-max_split) %>% droplevels() 
...
goal_1_dat_tb_num_splits <- tb_4_min_dat %>% dplyr::select(Video_name,split_number) %>%
  group_by(Video_name) %>% summarise(number_of_splits = max(split_number))
goal_1_dat_kf_num_splits <- kf_4_min_dat %>% dplyr::select(Video_name,split_number) %>%
  group_by(Video_name) %>% summarise(number_of_splits = max(split_number))
...
kf_4_min_dat <- kf_4_min_dat %>% filter(split_number <= 15, max_split >= 15) %>% dplyr::select(-max_split)
tb_4_min_dat <- tb_4_min_dat %>% filter(split_number <= 11, max_split >= 11) %>% dplyr::select(-max_split)
...
#long format
AvgN_half_kf = resamp_sum_all_mean_kf %>% dplyr::select(Video_name, Class, AvgN_mean, AvgN_bias, AvgN_CoV, AvgN_perc_diff,AvgN_bias_rel)
MaxN_half_kf = resamp_sum_all_mean_kf %>% dplyr::select(Video_name, Class, MaxN_mean , MaxN_bias, Maxn_CoV, MaxN_perc_diff,MaxN_bias_rel)
AvgN_half_tb = resamp_sum_all_mean_tb %>% dplyr::select(Video_name, Class, AvgN_mean, AvgN_bias, AvgN_CoV, AvgN_perc_diff,AvgN_bias_rel)
MaxN_half_tb = resamp_sum_all_mean_tb %>% dplyr::select(Video_name, Class, MaxN_mean , MaxN_bias, Maxn_CoV, MaxN_perc_diff,MaxN_bias_rel)
...
write.csv(resamp_sum_all_mean_long_tb,"results/60_seconds/goal_3_dat_tb.csv")
write.csv(resamp_sum_all_mean_long_kf,"results/60_seconds/goal_3_dat_kf.csv")
