...
goal_1_dat_tb <- read_csv("results/60_seconds/goal_1_2_4_dat_tb.csv")
goal_1_dat_kf <- read_csv("results/60_seconds/goal_1_2_4_dat_kf.csv")
...
#TB
ggplot(goal_1_dat_tb,aes(x=AvgN,y=jitter(MaxN,0.5),colour = Mobility:Schooling)) + geom_point(alpha=0.4) + 
  scale_y_log10(limits=c(0.8,30)) + scale_x_log10() + scale_color_brewer(palette = "Set2") + ylab("MaxN") + theme_bw()


#KF
ggplot(goal_1_dat_kf,aes(x=AvgN,y=jitter(MaxN,1.5),colour = Mobility:Schooling)) + geom_point(alpha=0.4) + 
...
ggplot(goal_1_dat_combined,aes(x=AvgN,y=jitter(MaxN,1.5),colour = Schooling:Mobility)) + geom_point(alpha=0.4) + 
  scale_y_log10(limits=c(0.8,50)) + scale_x_log10() + scale_color_brewer(palette = "Set2") + ylab("MaxN") +
  theme_bw()+ xlab("MeanCount") + 
  facet_grid(~data,scales="free") + theme(strip.background = element_blank(),
                                          strip.placement = "outside",
                                          panel.grid.major = element_blank(), 
                                          panel.grid.minor = element_blank(),
                                          strip.text.x = element_text(hjust=-0.01,size=11))



#ggsave("plots/MaxN_AvgN_scatter.png",height=5.12,width=5.79*(0.98/0.62))
...
ggplot(goal_1_dat_combined,aes(x=AvgN,y=jitter(MaxN,1.7),colour = Schooling:Mobility)) + geom_point(alpha=0.3,size=0.75) +
...
ggsave("plots/60_seconds/MaxN_AvgN_scatter_sims.png",height=1.1*5.12,width=1.1*5.79*(0.98/0.62))
