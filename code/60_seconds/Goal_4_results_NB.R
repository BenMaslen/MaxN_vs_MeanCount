...
for (i in c(1:length(species_kf))){
  ...
  kf_avgN_mod_full_temp <- glmmTMB(value_sum~year*river+(1|river:location)+(1|river:location:transect),
                                   ...
                                   family=nbinom2,offset=log(rep(56,286)))
...
kf_avgn_mod_full_temp <- glmmTMB(value_sum~year*river,
                                 data=goal_4_dat_kf %>% filter(Class == species_kf[i],measure=="AvgN"),
                                 family=nbinom1,offset=log(rep(56,286)),
                                 control=glmmTMBControl(optimizer=optim,optArgs = list(method="L-BFGS-B")))
kf_avgn_mod_null_temp <- glmmTMB(value_sum~1,
                                 data=goal_4_dat_kf %>% filter(Class == species_kf[i],measure=="AvgN"),
                                 family=nbinom1,offset=log(rep(56,286)),control=glmmTMBControl(optimizer=optim,optArgs = list(method="L-BFGS-B")))
test_statistic_dat_kf$AvgN[i] <- anova(kf_avgn_mod_null_temp,kf_avgn_mod_full_temp)$Chisq[2][1]
p_values_dat_kf$AvgN[i] <- anova(kf_avgn_mod_null_temp,kf_avgn_mod_full_temp)$`Pr(>Chisq)`[2][1]
...
kf_avgn_mod_full_temp <- glmmTMB(value_sum~year*river,
                                 data=goal_4_dat_kf %>% filter(Class == species_kf[i],measure=="AvgN"),
                                 family=nbinom2,offset=log(rep(56,286)),control=glmmTMBControl(optimizer=optim,optArgs = list(method="BFGS")))
kf_avgn_mod_null_temp <- glmmTMB(value_sum~1,
                                 data=goal_4_dat_kf %>% filter(Class == species_kf[i],measure=="AvgN"),
                                 family=nbinom2,offset=log(rep(56,286)),control=glmmTMBControl(optimizer=optim,optArgs = list(method="BFGS")))
test_statistic_dat_kf$AvgN[i] <- anova(kf_avgn_mod_null_temp,kf_avgn_mod_full_temp)$Chisq[2][1]
...
kf_avgn_mod_full_temp <- glmmTMB(value_sum~year*river,
                                 data=goal_4_dat_kf %>% filter(Class == species_kf[i],measure=="AvgN"),
                                 family=nbinom2,offset=log(rep(56,286)),control=glmmTMBControl(optimizer=optim,optArgs = list(method="L-BFGS-B")))
kf_avgn_mod_null_temp <- glmmTMB(value_sum~1,
                                 data=goal_4_dat_kf %>% filter(Class == species_kf[i],measure=="AvgN"),
                                 family=nbinom2,offset=log(rep(56,286)),control=glmmTMBControl(optimizer=optim,optArgs = list(method="L-BFGS-B")))
test_statistic_dat_kf$AvgN[i] <- anova(kf_avgn_mod_null_temp,kf_avgn_mod_full_temp)$Chisq[2][1]
...
for (i in c(1:length(species_kf))){
  ...
  tb_avgN_mod_full_temp <- glmmTMB(value_sum~bait*treatment+location+(1|location:block),
                                   ...
                                   family=nbinom2,offset=log(rep(42,42)))
