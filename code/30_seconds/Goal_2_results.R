...
goal_2_dat_tb <- read_csv("results/30_seconds/goal_2_dat_tb_long.csv")
goal_2_dat_kf <- read_csv("results/30_seconds/goal_2_dat_kf_long.csv")
...
sim_goal_2_dat <- read.csv("results/30_seconds/sim_goal_2_dat.csv")

#clean data
sim_goal_2_dat$Video_name <- factor(sim_goal_2_dat$Video_name)
sim_goal_2_dat$group <- "Simulations - unbaited"

# table(sim_goal_2_dat$measure)
# 
# str(sim_goal_2_dat)
# 
# sum(sim_goal_2_dat$value[1:14400001])
# 
sim_goal_2_dat$value[14400002]
# length(sim_goal_2_dat$value)

sim_goal_2_dat$value[sim_goal_2_dat$measure == "meancount"]
...
ggplot(goal_2_dat_combined_sims_filt,aes(x=split_number,y=value_tol,colour="Video:Species",group=Video_name:Class)) + 
  ...
  geom_line(data=goal_2_dat_sum_combined_sims,mapping=aes(x=split_number,y=value,group=measure,colour="Overall Avg"),
            size=1.65) + 
  xlab("video duration (minutes)") + ylab("value") + 
  scale_color_manual(values = cols,limits=c("Overall Avg","Avg per species","Video:Species"),
                     labels = c("Overall Avg","Avg per species","Video:Species")) +
  theme(axis.title.y=element_blank(),
        strip.background = element_blank(),
...
ggsave("plots/30_seconds/abund_video_length_sims.png",height=1.1*5.12,width=1.1*5.79*(0.98/0.62))
