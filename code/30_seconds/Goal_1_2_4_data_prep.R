...
#read data
kf_1_min_dat_original <- read_csv("data/Kakadu_fish_pred_1_min.csv")
tb_1_min_dat_original <- read_csv("data/Tassie_BRUV_RUV_pred_1_min.csv")

kf_1_min_dat_skipped <- read_csv("data/30_seconds/Kakadu_fish_pred_1_min.csv")
tb_1_min_dat_skipped <- read_csv("data/30_seconds/Tassie_BRUV_RUV_pred_1_min.csv")

#check they are ordered the same
sum(kf_1_min_dat_original$Class!=kf_1_min_dat_skipped$Class & 
      kf_1_min_dat_original$Video_name!=kf_1_min_dat_skipped$Video_name &
        kf_1_min_dat_original$split_number!=kf_1_min_dat_skipped$split_number)

sum(tb_1_min_dat_original$Class!=tb_1_min_dat_skipped$Class & 
      tb_1_min_dat_original$Video_name!=tb_1_min_dat_skipped$Video_name &
      tb_1_min_dat_original$split_number!=tb_1_min_dat_skipped$split_number)

#replace original MeanCount with skipped MeanCount
kf_1_min_dat_original$`Average Objects per frame` <- kf_1_min_dat_skipped$`Average Objects per frame`
tb_1_min_dat_original$`Average Objects per frame` <- tb_1_min_dat_skipped$`Average Objects per frame`

#rename
kf_1_min_dat = kf_1_min_dat_original
tb_1_min_dat = tb_1_min_dat_original
...
kf_1_min_dat <- kf_1_min_dat_max_split_temp %>% filter(split_number>4,split_number<max_split-6) %>% 
  droplevels() %>% dplyr::select(-max_split)  
...
goal_2_4_dat_tb_num_splits <- tb_1_min_dat %>% dplyr::select(Video_name,split_number) %>%
  group_by(Video_name) %>% summarise(number_of_splits = max(split_number))
goal_2_4_dat_kf_num_splits <- kf_1_min_dat %>% dplyr::select(Video_name,split_number) %>%
  group_by(Video_name) %>% summarise(number_of_splits = max(split_number))
...
kf_1_min_dat <- kf_1_min_dat %>% filter(split_number <= 56, max_split >= 56) %>% dplyr::select(-max_split)
tb_1_min_dat <- tb_1_min_dat %>% filter(split_number <= 42, max_split >= 42) %>% dplyr::select(-max_split)
...
write.csv(true_sum_all_kf,"results/30_seconds/goal_1_2_4_dat_kf.csv")
write.csv(true_sum_all_tb,"results/30_seconds/goal_1_2_4_dat_tb.csv")
...
write.csv(goal_2_4_dat_kf_long,"results/30_seconds/goal_1_2_4_dat_kf_long.csv")
write.csv(goal_2_4_dat_tb_long,"results/30_seconds/goal_1_2_4_dat_tb_long.csv")
...
tb_1_min_dat <- tb_1_min_dat %>% dplyr::select(-Class_video_name)

write.csv(tb_1_min_dat,"results/30_seconds/goal_2_dat_tb.csv")
...
kf_1_min_dat <- kf_1_min_dat %>% dplyr::select(-Class_video_name)

write.csv(kf_1_min_dat,"results/30_seconds/goal_2_dat_kf.csv")
...
goal_2_dat_kf_long <-  kf_1_min_dat %>% dplyr::select(-MaxN) %>%
  pivot_longer(
...
goal_2_dat_tb_long <-  tb_1_min_dat %>% dplyr::select(-MaxN) %>%
  pivot_longer(
    cols = c("AvgN","AvgN_cum_sum","MaxN_cum_sum"), 
    names_to = "measure",
    values_to = "value"
...
write.csv(goal_2_dat_kf_long,"results/30_seconds/goal_2_dat_kf_long.csv")
write.csv(goal_2_dat_tb_long,"results/30_seconds/goal_2_dat_tb_long.csv")
